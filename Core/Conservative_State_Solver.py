...
def U_next(U : np.array, X : np.array, intercell : str = "LF") : 
    """
    ...
    if intercell == "LF" :
        return np.array(U + (dt/dx)*(F12_Friedrich(U,X,dt,signe="-") - F12_Friedrich(U,X,dt,signe="+")))
    if intercell == "LW" :
        return np.array(U + (dt/dx)*(F12_Wendroff(U,X,dt) - F12_Wendroff(U,X,dt)))
    if intercell == "Go" : 
